...
- ADB path is a persisted, configurable setting behind `IAdbPathDiscovery`. A valid configured path is retained; discovery then checks the Portable `tools/adb` runtime, installed SDK Platform Tools/PATH and the ADB sibling/installation of MEmu. Manual selection remains the final fallback. The bundled Windows runtime contains only `adb.exe`, `AdbWinApi.dll`, `AdbWinUsbApi.dll` and their license/notice files; runtime discovery never downloads or installs software.
...
- `MainViewModel` vẫn là root ViewModel/DataContext duy nhất; constructor, public binding surface và helper dùng chung ở file root, còn implementation được nhóm bằng partial theo `Initialization`, `Devices`, `Scripts`, `Steps`, `Execution`, `Composite`, `Workspace`, `ControlCenter` và `EditorLifecycle`. Các partial chỉ tổ chức source, không tạo ViewModel con, forwarding surface hoặc lifecycle/state trùng lặp.
